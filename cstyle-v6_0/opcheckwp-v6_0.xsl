...
-->
	...
	<xsl:template match="faultreports">
			<xsl:if test="geninfo">
			<xsl:apply-templates select="geninfo | title"/>
		</xsl:if>
...
<!--		<xsl:template match="opcheckproc/faultreports//action">
	...
	<xsl:template match="opcheckproc">
			<fo:block font-size="10pt" font-weight="bold"  text-align="left" text-transform="uppercase" margin-bottom="4pt" margin-top="4pt" space-before.conditionality="discard" space-before.maximum="16pt" space-before.minimum="10pt" space-before.optimum="12pt">
...
<!--ELEMENT opcheck  -->
  ...
  <xsl:template match="checkstep">
				<fo:block>
		<fo:block space-after="10pt" space-before.conditionality="discard" space-before.maximum="16pt" space-before.minimum="10pt" space-before.optimum="12pt" span="all" xsl:use-attribute-sets="titlesub" keep-with-next="always">
	...
	<xsl:template match="step1" mode="CHECKSTEP" >
		...
		<fo:block span="all" font-size="10pt">
	...
	<xsl:template match="checkstep/indication" mode="CHECKSTEP">
		...
		<fo:block font-weight="bold" text-align="left" padding-after="4pt" padding-before="4pt" margin-bottom="4pt" margin-top="4pt" keep-with-next="always">CONDITION/INDICATION</fo:block>	
			<xsl:apply-templates select="node()" mode="CHECKSTEP"/>
		</fo:block>
	</xsl:template>
	
	<xsl:template match="checkstep/indication/para" mode="CHECKSTEP">
	...
	<xsl:template match="checkstep/malfunc" mode="CHECKSTEP">
		<fo:block margin-left="30pt">
		<fo:block font-weight="bold" text-align="left" padding-after="4pt" padding-before="4pt" margin-bottom="4pt" margin-top="4pt" text-transform="uppercase" space-after="12pt" space-before="12pt" keep-with-next="always">
	...
	<xsl:template match="checkstep/action" mode="CHECKSTEP">
		<fo:block margin-left="50pt">
		<fo:block font-weight="bold" text-align="left" padding-after="4pt" padding-before="4pt" margin-bottom="4pt" margin-top="4pt" keep-with-next="always">CORRECTIVE ACTION</fo:block>	
			<xsl:apply-templates select="node()" mode="CHECKSTEP"/>
			<xsl:call-template name="CHANGEBAR"/>
		</fo:block>
	</xsl:template>

	<xsl:template match="checkstep/action/para" mode="CHECKSTEP">
		<fo:block margin-left="10pt">
			<xsl:apply-templates select="."/>
		</fo:block>
	</xsl:template>
